...
export function letterFocus(){
    document.addEventListener('keydown', function (e) {
        ...
        const letteredAs = allAs.filter(a => {
            ...
            // text = text.replace(/\g&lt;/g, ' '); // Normalize spaces
            // Remove symbols like arrows or other non-alphanumeric characters from the beginning
            text = text.replace(/^[^\w]+/, ''); // Remove leading non-word characters
            console.log(text);
            return text.startsWith(key);
            // Fix getting text here
        });
